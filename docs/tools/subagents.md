...
- Default run timeout: if `sessions_spawn.runTimeoutSeconds` is omitted, OpenClaw uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `1800` seconds (30 minutes). Set `0` to disable the timeout.
...
- `runTimeoutSeconds?` (defaults to `agents.defaults.subagents.runTimeoutSeconds` when set, otherwise `1800`; set `0` for no timeout; when set, the sub-agent run is aborted after N seconds)
  ...
  agents: {
    defaults: {
      subagents: {
        ...
        runTimeoutSeconds: 1800, // default timeout for sessions_spawn when omitted (0 = no timeout)
      },
    },
  },
}
