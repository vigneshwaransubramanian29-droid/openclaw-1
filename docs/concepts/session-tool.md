...
- `runTimeoutSeconds?` (defaults to `agents.defaults.subagents.runTimeoutSeconds` when set, otherwise `1800`; set `0` for no timeout; when set, aborts the sub-agent run after N seconds)
